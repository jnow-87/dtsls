...
void symtab_print(){
	...
	list_for_each(files, file){
		...
		list_for_each(file->symbols, sym){
			if(strcmp(sym->file->path, file->path) != 0)
				LOG("    error: symbol file (%s) doesn't match file (%s)", sym->file->path, file->path);

			LOG("    %zu:%zu:%s: '%s'", sym->line, sym->column, sym->name, sym->signature);
		}

		list_for_each(file->header, file){
			LOG("    header: %s", file->path);
		}
	}
}
